...
#define 	MAX_IRBUF_SIZE				2048 //255//

#define REM_TAG                  "[Remote_ctl] "
#define REM_FUN(f)               printk( REM_TAG"%s\n", __FUNCTION__)
#define REM_LOG(fmt, args...)    printk( REM_TAG fmt, ##args)
#define REM_ERR(fmt, args...)    printk( REM_TAG"%s %d : "fmt, __FUNCTION__, __LINE__, ##args)

#define GPIO_REMOTEIR_RESET_PIN   (GPIO74 | 0x80000000)
#define GPIO_REMOTEIR_IRLED_PIN   (GPIO168 | 0x80000000)
...
static int __init remote_ctrl_probe(struct i2c_client *client,const struct i2c_device_id *id )
{
	...
	deviceData->init_gpio	= GPIO_HALL_4_PIN;
...
//	GPIO_HALL_4_PIN
	...
	//	return -2;
	//}
	mt_set_gpio_out(GPIO_REMOTEIR_IRLED_PIN,GPIO_OUT_ONE);
	if( deviceData->init_gpio ) {
		mt_set_gpio_mode(GPIO_HALL_4_PIN, 0);
		mt_set_gpio_dir(GPIO_HALL_4_PIN,GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO_HALL_4_PIN,GPIO_OUT_ONE);
		_i2c_gpio_direction = RC_GPIO_MODE_OUTPUT;
		_i2c_gpio_value	 	= 1;
	}

	if( deviceData->reset_gpio ) {
		mt_set_gpio_mode(GPIO_REMOTEIR_RESET_PIN,GPIO_HALL_1_PIN_M_GPIO);
...
static int __exit remote_ctrl_remove(struct i2c_client *client)
{
	...
	//xuxiaojiang
	mt_set_gpio_mode(GPIO_HALL_4_PIN, 0);
	mt_set_gpio_out(GPIO_HALL_4_PIN,GPIO_OUT_ZERO);
	mt_set_gpio_pull_enable(GPIO_HALL_4_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_HALL_4_PIN, GPIO_PULL_UP);
	mt_set_gpio_dir(GPIO_HALL_4_PIN,GPIO_DIR_IN);
...
{
	REM_FUN();
	//mt_set_gpio_out(GPIO_REMOTEIR_IRLED_PIN,GPIO_OUT_ZERO);
	return 0;	
}
				...
				loff_t *ppos)
{
	...
	if( gDeviceData->init_gpio ) {
		...
		//if( (_i2c_gpio_direction != RC_GPIO_MODE_OUTPUT) || (rc != 1) ) 
		{
			mt_set_gpio_mode(GPIO_HALL_4_PIN, 0);
			mt_set_gpio_dir(GPIO_HALL_4_PIN,GPIO_DIR_OUT);
			mt_set_gpio_out(GPIO_HALL_4_PIN,0);	//xuxiaojiang

			msleep(10);
			mt_set_gpio_out(GPIO_HALL_4_PIN,1);	//xuxiaojiang
			msleep(10);
...
static int remote_ctrl_ioctl(struct file * file, unsigned int cmd, unsigned long arg)
{
	...
	case REMOTE_CTRL_CTL_SET_GPIO_MODE:
		...
		if( arg == RC_GPIO_MODE_INPUT )
		{
			...
			err = mt_get_gpio_in(GPIO_HALL_4_PIN);
		}
		else if( arg == RC_GPIO_MODE_OUTPUT )
		{
			mt_set_gpio_dir(GPIO_HALL_4_PIN,GPIO_DIR_OUT);
			mt_set_gpio_out(GPIO_HALL_4_PIN, 1);
			_i2c_gpio_direction = RC_GPIO_MODE_OUTPUT;
	...
	case REMOTE_CTRL_CTL_SET_GPIO_VALUE:
		mt_set_gpio_out(GPIO_HALL_4_PIN, (int)arg);
	...
	case REMOTE_CTRL_CTL_MODE_BOOTLOADER:
		...
		*/
		...
		{
			mt_set_gpio_out(GPIO_HALL_4_PIN,_i2c_gpio_value);
		}
	...
	case REMOTE_CTRL_CTL_MODE_USERIR:
		...
		if(_i2c_gpio_direction != RC_GPIO_MODE_OUTPUT ) {
			mt_set_gpio_dir(GPIO_HALL_4_PIN,GPIO_DIR_OUT);
			mt_set_gpio_out(GPIO_HALL_4_PIN, 0);
		}
		mt_set_gpio_out(GPIO_HALL_4_PIN, 0);
		msleep(1);
